...
/**
 * Fetches all users and returns them sorted by username.
...
export async function getAllUsers() {
  try {
    ...
    let documents = listDocuments.documents.sort((a, b) =>
      a.username.localeCompare(b.username)
    );

    // const players = documents.map((player) => new User(player));
...
export const updateRating = async (id, rating) => {
  const updatedData = {
    rating: rating,
    timestamp: getFormattedDate(), // formattedDate should be defined as you already have in your code
  };

  try {
    const newRating = await databases.updateDocument(
      appwriteConfig.databaseId,
      appwriteConfig.ratingsCollectionId,
      id,
      updatedData
...
export async function getAllRatingsForLoggedInUser() {
  try {
    // Get loggedin user
    const loggedInUser = await account.get();
    ...
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.ratingsCollectionId,
      [Query.equal("ratedByPlayerId", loggedInUserId)]
    );
...
export async function getAllRatingsForDocId(id) {
  try {
    const ratings = await databases.getDocument(
      appwriteConfig.databaseId,
      appwriteConfig.ratingsCollectionId,
      id
    );
